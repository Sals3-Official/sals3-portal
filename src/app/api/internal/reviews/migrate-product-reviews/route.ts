...
/**
 * POST /api/internal/reviews/migrate-product-reviews — one-time DDL for buyer
 * product reviews (`sals3_product_reviews`, `sals3_product_review_replies`).
 *
 * Same break-glass pattern as
 * `/api/internal/orders/migrate-order-line-snapshot` and
 ...
 * Nothing in the change that adds this route touches either table; the review
 * domain, its actions, and its screens are separate changes that follow only
 * once a run here reports both tables in `tablesExistAfter`.
 *
 * Idempotent — see `migrateProductReviews`. Safe to call more than once.
 */
...
/**
 * GET — read-only. Reports whether the tables actually exist, without writing
...
export async function GET(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    const tablesExist = await readExistingReviewTables(getDb());

    return NextResponse.json({ ok: true, tablesExist }, { headers: NO_STORE });
  } catch (error) {
    // eslint-disable-next-line no-console
    console.error('[portal] product-reviews status check failed', {
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    const { migrateProductReviews } =
      await import('@/modules/reviews/migrate-product-reviews');
    const result = await migrateProductReviews(getDb());

    return NextResponse.json(result, { headers: NO_STORE });
  } catch (error) {
    // eslint-disable-next-line no-console
    console.error('[portal] product-reviews migration failed', {
      error: error instanceof Error ? error.message : 'unknown',
    });
