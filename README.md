...
listed variant. The per-row editor clamps equal-to-cost entries up to the next
minor unit, the bulk price dialog refuses a value below that same floor, and
`publishProduct` refuses the value again server-side; the manual seller-price
path cannot record a zero-spread offer as resolved.
